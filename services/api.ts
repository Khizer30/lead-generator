...
export const api = {
  ...
  updateEmailSettings: async (updates: Partial<EmailSettings>): Promise<EmailSettings> => {
    return mockFetch(() => {
      const current = JSON.parse(
        localStorage.getItem(STORAGE_KEYS.EMAIL_SETTINGS) || JSON.stringify(INITIAL_EMAIL_SETTINGS)
      );
  ...
  addOwner: async (
    ownerData: Omit<Owner, "id" | "avatar" | "status" | "inviteToken" | "tokenExpiresAt">
  ): Promise<Owner> => {
    const owners = await api.getOwners();
    const initials = ownerData.name
      .split(" ")
      .map((n) => n[0])
      .join("")
      .toUpperCase()
  ...
  createProject: async (projectData: Omit<Project, "id" | "createdAt">, leadIds: string[]): Promise<Project> => {
    ...
    const updatedLeads = leads.map((l) =>
      leadIds.includes(l.id) ? { ...l, projectId: newProject.id, updatedAt: new Date().toISOString() } : l
    );
