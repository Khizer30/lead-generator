...
const EmailSettingsDashboard: React.FC<EmailSettingsDashboardProps> = ({ lang }) => {
  ...
  const checkConnection = async () => {
    ...
    try {
      ...
      setFeedback({
        type: "error",
        message: lang === "de" ? "Verbindungsprüfung fehlgeschlagen." : "Connection check failed."
      });
    } finally {
      setChecking(false);
    }
  ...
  const sendTest = async () => {
    ...
    try {
      ...
      setFeedback({
        ...
        message:
          lang === "de"
            ? `Testmail wurde erfolgreich an ${testEmail} gesendet.`
            : `Test mail successfully sent to ${testEmail}.`
      });
    } catch (e) {
  ...
  return (
    <div className="flex-1 flex flex-col p-8 bg-white/50 backdrop-blur-sm rounded-3xl m-4 shadow-inner overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-8">
        {feedback && (
          <div
            className={`p-4 rounded-2xl flex items-start gap-3 animate-in fade-in slide-in-from-top-4 ${
              ...
              {settings.provider === EmailProvider.WORKSPACE_GMAIL_API && (
                <div>
                  ...
                  <p className="text-[10px] text-gray-400 mt-2">
                    {lang === "de"
                      ? "Stellen Sie sicher, dass Domain-Wide Delegation für den Scope"
                      : "Ensure Domain-Wide Delegation is enabled for scope"}{" "}
                    <code className="bg-gray-100 px-1 rounded">gmail.send</code>{" "}
                    {lang === "de" ? "aktiviert ist." : "activated."}
                  </p>
                </div>
              )}
            </div>
              ...
              <div
                className={`p-4 rounded-xl border flex items-start gap-4 ${
                  settings.lastCheckStatus === "ok"
                    ...
                    : settings.lastCheckStatus === "error"
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1 leading-relaxed">
                    {settings.lastCheckMessage ||
                      (lang === "de" ? "Noch keine Prüfung durchgeführt." : "No checks performed yet.")}
                  </p>
                  <button
                    type="button"
            ...
            <div className="bg-blue-50 p-6 rounded-2xl border border-blue-100">
              ...
              <ul className="text-xs text-blue-700 space-y-3">
                <li className="flex gap-2">
                  <span className="w-4 h-4 rounded-full bg-blue-200 flex items-center justify-center shrink-0 font-bold">
                    1
                  </span>
